    ...
    namespace frontend
    {
        ...
        bool FrontEndPDPD::supported_impl(const std::vector<std::shared_ptr<Variant>>& variants) const
        {
            ...
            if (is_type<VariantWrapper<std::string>>(variants[0]))
            {
                ...
                if (model_path.length() < suffix.length() ||
                    (0 != model_path.compare(
                              model_path.length() - suffix.length(), suffix.length(), suffix)))
                {
            ...
            FrontEndPDPD::load_impl(const std::vector<std::shared_ptr<Variant>>& variants) const
        {
            ...
            else if (variants.size() == 2)
            {
                ...
                {
                    auto m_stream = as_type_ptr<VariantWrapper<std::shared_ptr<std::istream>>>(variants[0])->get();
                ...
                {
                    auto w_stream =
                        as_type_ptr<VariantWrapper<std::shared_ptr<std::istream>>>(variants[1])->get();
