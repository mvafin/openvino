...
from mo.front.torchscript.extractor import onnx_op_extractor, onnx_op_extractors
from mo.front.torchscript.loader import load_torchscript_model, protobuf2nx
from mo.graph.graph import Graph
from mo.utils.error import Error
from mo.utils.utils import refer_to_faq_msg


class TorchScriptLoader(Loader):
    enabled = True

    def load(self, graph: Graph):
        argv = graph.graph['cmd_params']
        model_proto = load_torchscript_model(argv.input_model)
        ...
        try:
            ...
            raise Error(
                'Cannot pre-process TorchScript graph after reading from model file "{}". ' \
