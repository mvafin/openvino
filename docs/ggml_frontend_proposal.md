# Proposal: A GGML Frontend for OpenVINO with Two Input Paths
...
Add a first-class **GGML frontend** to OpenVINO (`src/frontends/ggml/`) that converts
...
```
                ┌──────────────────────────────────────────────┐
                │  src/frontends/ggml/  (OpenVINO, core-only)    │
                │                                                │
...
|---|---|---|
| Move translators + interface into `src/frontends/ggml/` | relocation; CMake via `ov_add_frontend`; namespace/visibility | ~2,400 LoC moved |
...
1. **M1 — Frontend skeleton + cgraph path.** Move translators + interface into
   `src/frontends/ggml/`; llama.cpp links it back and deletes its copy. Validates the
