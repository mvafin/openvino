# GGUF Frontend — Implementation Plan (qwen3-8b bring-up)

**Status:** Plan for execution
**Date:** 2026-06-10
**Companion doc:** [gguf_frontend_proposal.md](gguf_frontend_proposal.md) (design rationale & architecture)
...
- `src/frontends/gguf/` already contains the **translators + interface** copied from
...
Goal: `src/frontends/gguf` compiles inside OpenVINO as a real frontend, validated by the
cgraph path (llama.cpp links it back). No `.gguf` reading yet.

### M1.1 Public-header layout & namespace
- Create `src/frontends/gguf/include/openvino/frontend/ggml/`:
...
- Move implementation files under `src/frontends/gguf/src/` (mirror the other FEs:
...
### M1.2 CMake
- Add `src/frontends/gguf/CMakeLists.txt` using `ov_add_frontend(NAME ggml ...)`
...
- Add `ENABLE_OV_GGUF_FRONTEND` option (default ON) and wire it into
...
- `src/frontends/gguf/src/builder/`:
