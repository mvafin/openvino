...
#
# M3 IO adapter: wrap the GGML-frontend model (ggml IO: inp_tokens / inp_pos /
...
#
# The adapter prepends a small subgraph that derives the ggml inputs from the genai
...
def adapt_to_genai(model):
    ...
    for required in ("inp_tokens", "inp_pos", "self_kq_mask", "token_len_per_seq"):
        assert required in params, f"frontend model missing ggml input {required!r}"
    ...
    # ---- rewire: replace the ggml Parameters' uses with the computed tensors ----
    replacements = {
