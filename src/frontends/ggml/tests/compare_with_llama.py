...
#
# Compare the OpenVINO GGML frontend against native llama.cpp on a GGUF model.
#
# Greedy-decodes a prompt through the model produced by `core.read_model("*.gguf")`
# (the GGML frontend) and prints the generated text / token ids, so it can be diffed
# against `llama-simple -m model.gguf <prompt>` (which also greedy-decodes).
#
# The frontend's model uses the ggml IO contract (inp_tokens / inp_pos / self_kq_mask /
...
def build_inputs(tokens, past_len):
    """Build the ggml-IO tensors for one decode step.
