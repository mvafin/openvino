...
// ("GGML_OP_MUL_MAT", "GGML_OP_ROPE", ...). It mirrors exactly what the GgmlDecoder
...
// The whole model as a flat, topologically-ordered list of GgmlOp nodes plus the
...
struct GgmlGraph {
    std::vector<GgmlOp> nodes;

    // Model inputs (Parameters), extra inputs (e.g. attention_size; Parameter or Constant),
    // and weights (Constants). Same semantics as the corresponding GgmlDecoder getters.
