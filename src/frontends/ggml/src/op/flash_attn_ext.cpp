...
OutputVector translate_flash_attn_ext(const NodeContext& context) {
    ...
    auto tile_kv = [&](int64_t num_heads, int64_t num_heads_kv, int64_t head_size, ov::Output<Node> kv) {
        ...
        if (factor > 1 && num_heads_kv > 1) {
            ...
            auto kv_broadcast_shape = ov::op::v0::Constant::create(
                ov::element::i64, {5}, {(int64_t)1, (int64_t)1, (int64_t)1, factor, (int64_t)1});
            auto new_kv_shape =
    ...
    if (!has_sinks) {
        ...
        auto sink_f16 = sink.get_element_type() != element::f16
                            ...
                            : sink;
        auto sink_shape =
            v0::Constant::create(element::i64, {4}, std::vector<int64_t>{1, (int64_t)q_shape[2], 1, 1});
        auto sink_r = std::make_shared<v1::Reshape>(sink_f16, sink_shape, false);
        sdpa = std::make_shared<ov::op::v13::ScaledDotProductAttention>(
            q_t, k_t, v_t, mask_sliced, scale_node, sink_r, false);
    }
