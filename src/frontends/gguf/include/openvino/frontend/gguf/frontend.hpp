...
public:
    ...
    /// - `ov::frontend::ConversionExtension` — registers a custom op translator
    ///   for the GGML op name given by `get_op_type()`.  The converter receives an
    ///   `ov::frontend::gguf::NodeContext` and returns an `ov::OutputVector`.
    /// - `ov::frontend::TelemetryExtension` — receives error / event callbacks for
    ///   conversion diagnostics.
    /// - `ov::detail::SOExtension` — shared-library extension; its inner extension
    ///   is recursively registered.
...
protected:
    ...
    /// \param variants Either a GgufDecoder (cgraph / gguf-builder path) or a path to a
    /// .gguf file.
