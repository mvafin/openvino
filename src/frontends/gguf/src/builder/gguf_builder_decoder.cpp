...
std::vector<size_t> GgufBuilderDecoder::get_input_stride(const std::string& name) const {
    const auto& m = node().input_strides;
    auto it = m.find(name);
    OPENVINO_ASSERT(it != m.end(), "[gguf] no input stride for '", name, "'");
    return it->second;
}

int64_t GgufBuilderDecoder::get_input_view_offset(const std::string& name) const {
    const auto& m = node().input_view_offsets;
    auto it = m.find(name);
    return it == m.end() ? 0 : it->second;
}

element::Type GgufBuilderDecoder::get_input_type(const std::string& name) const {
    const auto& m = node().input_types;
    auto it = m.find(name);
    OPENVINO_ASSERT(it != m.end(), "[gguf] no input type for '", name, "'");
    return it->second;
}
...
element::Type GgufBuilderDecoder::get_output_type() const {
...
int GgufBuilderDecoder::get_op_case() const {
    return node().op_case;
}
