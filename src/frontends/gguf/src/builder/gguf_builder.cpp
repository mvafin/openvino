...
public:
    TransformerBuilder(const std::map<std::string, GGUFMetaData>& config,
        ...
        m_is_geglu = arch_str == "gemma" || arch_str == "gemma2";
        ...
        m_has_swa = m_has_sinks;                     // sink attention co-occurs with SWA layer alternation
...
std::shared_ptr<GgufGraph> TransformerBuilder::build() {
    ...
    for (int il = 0; il < m_n_layer; ++il) {
        ...
        // gpt-oss SWA: even layers (by default) use the sliding-window attention mask and a
        // distinct RoPE frequency base. Matches llama.cpp set_swa_pattern(period, dense_first=false):
        // layer il is SWA if il % period < period - 1.
        const bool is_swa_layer =
            m_has_swa && m_swa_layer_pattern > 0 && (il % m_swa_layer_pattern < (m_swa_layer_pattern - 1));
        RopeConfig rope_config_l = m_graph->rope_config;
        if (is_swa_layer && m_rope_freq_base_swa != m_graph->rope_config.freq_base) {
            rope_config_l.freq_base = m_rope_freq_base_swa;
        }
...
std::shared_ptr<GgufGraph> build_ggml_graph_from_gguf(const std::string& file) {
    ...
    OPENVINO_ASSERT(supported_archs().count(arch),
                    ...
                    arch,
                    "'. Supported: llama, qwen2, qwen3, phi3, minicpm, hunyuan-dense, gemma, gemma2.");
