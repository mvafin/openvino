...
std::shared_ptr<Model> TranslateSession::translate_graph(const frontend::InputModel::Ptr& input_model) {
    ...
    std::shared_ptr<GgufDecoder> ggml_model_decoder = ggml_model->get_model_decoder();
    ...
    auto ggml_model_decoder = std::dynamic_pointer_cast<InputModel>(m_input_model)->get_model_decoder();
    {
        ...
        if (ggml_model_decoder->is_stateful()) {
            ...
            for (size_t i = 0; i < model->get_output_size(); i++) {
                ...
                auto it = model_output_indexes.find(output_friendly_name);
                OPENVINO_ASSERT(it != model_output_indexes.end(),
                                "[ggml] model output '",
                                output_friendly_name,
                                "' not found in decoder output names");
                auto output_id = it->second;
