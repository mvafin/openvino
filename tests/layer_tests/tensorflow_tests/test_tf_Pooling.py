...
class TestPooling(CommonTFLayerTest):
    def create_pooling_net(self, kernel_size, strides, pads, in_shape, out_shape, method, ir_version, use_mo_extractors):
        """
    ...
    @pytest.mark.nightly
    def test_pool_4D(self, params, ie_device, precision, ir_version, temp_dir, use_mo_extractors):
        self._test(*self.create_pooling_net(**params, ir_version=ir_version, use_mo_extractors=use_mo_extractors),
    ...
    @pytest.mark.nightly
    def test_pool_5D(self, params, ie_device, precision, ir_version, temp_dir, use_mo_extractors):
        if ie_device == 'GPU':
