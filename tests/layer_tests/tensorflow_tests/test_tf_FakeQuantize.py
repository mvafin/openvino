...
class TestFakeQuantize(CommonTFLayerTest):
    ...
    def create_fake_quantize_net(self, il, ih, num_bits, narrow_range, nudged_il, nudged_ih, expected_step, ir_version, use_mo_extractors):
        # original tf model
    ...
    @pytest.mark.nightly
    def test_fake_quantize(self, params, ie_device, precision, ir_version, temp_dir, use_mo_extractors):
        self._test(*self.create_fake_quantize_net(**params, ir_version=ir_version, use_mo_extractors=use_mo_extractors), ie_device, precision, ir_version,
                   kwargs_to_prepare_input=params, temp_dir=temp_dir, use_mo_extractors=use_mo_extractors)
